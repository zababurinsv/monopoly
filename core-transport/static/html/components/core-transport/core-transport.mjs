...
import Libp2pBundle from '/static/html/components/component_modules/bundle/libp2p/libp2p.mjs'

customElements.define('core-transport',
    class extends HTMLElement {
        ...
        constructor () {
            ...
            async function modules (obj) {
                let bundle = Libp2pBundle.default
                events.eventListener.get(true, 'await', '5', '','/transport',async (event)=>{
                        object.callback(event)
                })


                let libp2p = await bundle.Libp2p.create({
                    addresses: {
                    ...
                    modules: {
                        transport: [bundle.Websockets, bundle.WebRTCStar],
                        connEncryption: [bundle.NOISE, bundle.Secio],
                        streamMuxer: [bundle.Multiplex],
                        peerDiscovery: [bundle.Boostrap]
                    },
                    config: {
                        peerDiscovery: {
                            bootstrap: {
                ...
                // UI elements
                const status = obj['this']['shadowRoot'].getElementById('status')
                const output = obj['this']['shadowRoot'].getElementById('output')



                const addr = bundle.multiaddr('/ip4/0.0.0.0/tcp/9090/wss/p2p-webrtc-star')

                const ws = new bundle.WebRTCStar({
                    upgrader: {
                ...
                const values = await bundle.pipe(
                    socket,
                    bundle.collect
                )
                ...
                await libp2p.start()
                status.innerText = 'p2p started!'
                log(`id is ${libp2p.peerId.toB58String()}`)
